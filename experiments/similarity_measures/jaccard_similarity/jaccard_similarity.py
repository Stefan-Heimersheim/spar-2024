...
from similarity_measures import JaccardSimilarityAggregator
...
for layer in [9, 10]: # range(model.cfg.n_layers - 1):
    ...
    jaccard_similarities = run_with_aggregator(model, saes, 'hook_resid_pre', tokens, aggregator)

    np.savez_compressed(output_filename_fn(layer), jaccard_similarities)
